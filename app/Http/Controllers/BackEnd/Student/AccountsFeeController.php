...
class AccountsFeeController extends Controller
{
    public function view(){
    	$data['allData'] = StudentAccountFee::all();
    	return view('backend.student_account.Accounts-fee-view',$data);
    }
    public function add(){
    	$data['years'] = StudentYear::orderBy('id','DESC')->get();
    	$data['classes'] = StudentClass::all();
    	$data['fee_categories'] = FeeCategory::all();
    	return view('backend.student_account.Accounts-fee-add',$data);
    }
    public function GetStudent(Request $request){
        ...
        foreach ($data as $key => $std) {
            ...
            $accountstudentfees = StudentAccountFee::where('student_id',$std->student_id)->where('year_id',$std->year_id)->where('fee_category_id',$fee_category_id)->where('class_id',$std->class_id)->where('date',$date)->first();
    ...
    public function store(Request $request){
    	$date = date('Y-m',strtotime($request->date));
    	StudentAccountFee::where('year_id',$request->year_id)->where('class_id',$request->class_id)->where('fee_category_id',$request->fee_category_id)->where('date',$date)->delete();
    	$checkdata = $request->checkmange;
    	  if ($checkdata !=null) {
    	...
    	if (!empty(@$data) || empty($checkdata)) {
    		...
    		return redirect()->back()->with('error','sorr! data no saved');
    	}
    }
