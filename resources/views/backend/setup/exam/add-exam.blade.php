...
<div class="card col-md-6 m-3 pt-3">
    <div class="card-header">
        @if(isset($editData))
        <strong>Edit</strong> exam type
        @else
        <strong>Add</strong>  exam type
        @endif

        <a href="{{route('view-exam')}}"  type="button" class="btn btn-primary float-right active col-md-4">
          <i class="fa fa-list"></i>  exam List
      	 </a>
     </div>
     <form action="{{@$editData?route('update-exam',$editData->id):('store-exam')}}" method="post" id="myForm" enctype="multipart/form-data" class="">
       	@csrf
